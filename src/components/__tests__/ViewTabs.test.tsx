...
const enabled = { overview: true, prd: true, srs: true, vtp: true, testing: true, jobs: true, arch: true, sdd: false, risk: false, soup: false, threat: false, controls: false, sec: false, reference: false, releases: true, audit: true, trace: true };
...
describe('ViewTabs', () => {
  ...
  it('orders the tabs chronologically through the design-control phases', () => {
    ...
    expect(labels).toEqual(['Overview', 'Auditor', 'PRD', 'SRS', 'Refs', 'SAD', 'SDD', 'SOUP', 'Security', 'Risk', 'Threats', 'Controls', 'VTP', 'Results', 'Traceability', 'Releases', 'Jobs']);
  });
  ...
  it('labels each design-control phase with a band (Design Inputs spans the requirements tabs)', () => {
    ...
    // "Design Verification" spans VTP + Results — columns 13–14: Design Outputs covers
    ...
    expect(verification.style.gridColumn).toBe('13 / span 2');
  });
  ...
  it('disables a tab whose document is absent and does not select it', () => {
    const onSelect = vi.fn();
    render(
      <ViewTabs current="srs" enabled={{ overview: true, prd: false, srs: true, vtp: false, testing: false, jobs: false, arch: false, sdd: false, risk: false, soup: false, threat: false, controls: false, sec: false, reference: false, releases: false, audit: false, trace: false }} onSelect={onSelect} />,
    );
