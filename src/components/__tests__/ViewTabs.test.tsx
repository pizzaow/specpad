...
describe('ViewTabs', () => {
  ...
  it('orders the tabs chronologically through the design-control phases', () => {
    ...
    expect(labels).toEqual(['Overview', 'Product Requirements', 'Software Requirements', 'Architecture', 'Verification Tests', 'Results', 'Auditor', 'Traceability', 'Releases', 'Jobs']);
  });
  ...
  it('labels each design-control phase with a band (Design Inputs spans the requirements tabs)', () => {
    ...
    // "Design Verification" spans Verification Tests + Results (columns 5–6).
    const verification = [...container.querySelectorAll('.phase-band')].find((b) => b.textContent === 'Design Verification') as HTMLElement;
    expect(verification.style.gridColumn).toBe('5 / span 2');
  });
