...
describe('ViewTabs', () => {
  const enabled = { overview: true, srs: true, vtp: true, testing: true, jobs: true, arch: true, releases: true, audit: true };
  ...
  it('leads with Overview and separates the tab groups', () => {
    ...
    expect(labels).toEqual(['Overview', 'Requirements', 'Verification Tests', 'Results', 'Architecture', 'Auditor', 'Releases', 'Jobs']);
  });
  ...
  it('disables a tab whose document is absent and does not select it', () => {
    const onSelect = vi.fn();
    render(
      <ViewTabs current="srs" enabled={{ overview: true, srs: true, vtp: false, testing: false, jobs: false, arch: false, releases: false, audit: false }} onSelect={onSelect} />,
    );
