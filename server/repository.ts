/**
 * The repository: one bare clone, plus one sparse-checked-out worktree per user.
 *
...
export class Repository {
  ...
  constructor(
    ...
    this.runner = runner;
    this.bareDir = path.join(config.workDir, 'repo.git');
  ...
  async ensureClone(): Promise<void> {
    ...
    if (!exists) {
      const git = new Git(this.runner, this.config.workDir);
      await git.run('clone', '--bare', this.config.repo.url, this.bareDir);
  ...
  private async provision(dir: string): Promise<WorkingCopy> {
    ...
    if (!ready) {
      ...
      const branch = `origin/${this.config.repo.branch}`;
      ...
      await tree.run('sparse-checkout', 'set', ...this.config.repo.paths);
      await tree.run('checkout', '--detach', branch);
    }

    return new WorkingCopy(new Git(this.runner, dir), this.config, dir);
  }
